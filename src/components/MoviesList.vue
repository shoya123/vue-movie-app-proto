  ...
  <div class="wrapper" v-if="listLoaded">
    <div class="movies" v-if="movies.length">
      ...
      <div class="movies__nav" v-if="!shortList" :class="{'is-hidden' : currentPage == pages}">
        <button @click="loadMore" class="button">もっと見る</button>
      </div>
    </div>
...
export default {
  ...
  computed: {
    ...
    countResults(){
      if(this.results > 1){
        return numeral(this.results).format('0,0') + '作品';
      } else {
        return numeral(this.results).format('0,0') + '作品';
      }
    }
    ...
    updateFavorite(){
      if(this.mode == 'favorite'){
        ...
        for(let i = 1; i <= this.currentPage; i++){
          promises.push(axios.get(`https://api.themoviedb.org/3/account/${storage.userId}/favorite/movies?api_key=${storage.apiKey}&session_id=${storage.sessionId}&language=ja&sort_by=created_at.desc&page=${i}`))
        }
